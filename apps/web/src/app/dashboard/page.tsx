...
const demoTexts: Record<string, string> = {
  ...
  communications:
    "Decision: ship the smaller importer on Friday. Owner: Riya. Blocker: CSV date parser — Amit patches by 4 PM, QA retests the 12 failing rows. Fallback: Saturday maintenance window if patch isn't ready. The impact of missing this deadline is that marketing cannot launch the new campaign on Monday. We have informed the stakeholders of this potential risk. The database migration will be monitored by the site reliability team. All developers must avoid deploying non-critical changes until this feature is stable. Daily stand-ups will focus exclusively on remaining blockers. Post-mortem will be scheduled for next week to analyze the delays.",
  content:
...
function Spinner() {
  return (
    <svg className="spinner-svg" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <path className="spinner-arc" d="M12 2a10 10 0 0 1 10 10" stroke="#57f1db" strokeWidth="3" strokeLinecap="round" />
    </svg>
  );
}
    ...
    try {
      if (compareMode) {
        const [res1, res2] = await Promise.all([
          postJson("/score/text", { text, domain }),
          postJson("/score/text", { text: compareText, domain })
        ]);
        setResult(res1);
        setCompareResult(res2);
        await postJson("/events/score", { url: "dashboard://scan-compare", title: "Dashboard Compare Scan", domain, score: res1.score, oversight: res1.oversight });
      } else {
      ...
      <div className="site-nav-links">
        ...
        {user ? (
          <div className="site-nav-user" style={{ display: "flex", alignItems: "center", gap: "8px", background: "rgba(255,255,255,0.05)", padding: "4px 12px", borderRadius: "100px", border: "1px solid rgba(255,255,255,0.1)" }}>
            ...
            <span style={{ fontSize: "12px", fontFamily: "var(--mono)", color: "#dde4e0" }}>{user.login}</span>
            <button onClick={() => setUser(null)} style={{ background: "none", border: "none", color: "#859490", cursor: "pointer", padding: "0 0 0 8px", fontSize: "11px" }}>Out</button>
          </div>
        ) : (
          <button onClick={() => setUser({ login: "demo-judge", avatar_url: "https://github.com/identicons/demo-judge.png" })} className="site-nav-login" style={{ background: "#24292e", color: "white", border: "1px solid #1b1f23", padding: "6px 12px", borderRadius: "6px", fontSize: "12px", cursor: "pointer", fontWeight: "600" }}>Login with GitHub</button>
        )}
      </div>
    </nav>
    <main>
      ...
      {tab === "scan" && (
        <section className="workspace">
          <div className="panel" style={{ gridColumn: compareMode ? "1 / -1" : undefined }}>
            <div className="toolbar" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
                ...
                <button 
                  ...
                  style={{ 
                    background: compareMode ? "rgba(139, 92, 246, 0.2)" : "rgba(255,255,255,0.05)",
                    border: compareMode ? "1px solid rgba(139, 92, 246, 0.5)" : "1px solid rgba(255,255,255,0.1)",
                    color: compareMode ? "#a78bfa" : "#859490"
                  }}
                >
                  {compareMode ? "Exit Compare Mode" : "Compare Mode"}
                </button>
              </div>
              <div className="demo-presets-scan" style={{ visibility: compareMode ? "hidden" : "visible" }}>
                <button className="demo-preset-btn" onClick={() => setText("Refactored the authentication module because it was causing performance issues in production. The new implementation is more robust and provides better error handling for various edge cases.")}>Slop Example</button>
                <button className="demo-preset-btn" onClick={() => setText("Profiling showed auth middleware adding 340ms to every request. Moved token validation from the hot path to a background job using Redis cache. P95 latency dropped from 420ms to 85ms.")}>Genuine Example</button>
              </div>
            ...
            {compareMode && (
              <div style={{ padding: "0 20px 12px", color: "#a78bfa", fontSize: "13px" }}>
                <strong>Improvement Engine Demo:</strong> Paste the original AI draft on the left, and your revised version on the right to see the oversight improvement.
              </div>
            )}

            <div style={{ display: "flex", gap: "12px", width: "100%", height: "200px" }}>
              <textarea 
                style={{ flex: 1, height: "100%", minHeight: "200px" }} 
                value={text} 
                onChange={(event) => setText(event.target.value)} 
                placeholder={compareMode ? "Paste original draft here..." : "Paste your text here..."}
              />
              {compareMode && (
                <textarea 
                  style={{ flex: 1, height: "100%", minHeight: "200px", borderLeft: "2px solid rgba(139,92,246,0.3)" }} 
                  value={compareText} 
                  onChange={(event) => setCompareText(event.target.value)} 
                  placeholder="Paste revised version here..."
                />
              )}
            </div>
          ...
          <div className="panel" style={{ gridColumn: compareMode ? "1 / -1" : undefined }}>
            <h2>{compareMode ? "Side-by-Side Signal Breakdown" : "Signal Breakdown"}</h2>
            
            <div style={{ display: "flex", gap: "24px" }}>
              <div style={{ flex: 1 }}>
                {compareMode && <h3 style={{ color: "#859490", borderBottom: "1px solid rgba(255,255,255,0.1)", paddingBottom: "8px" }}>Original Draft</h3>}
                {loading === "scan" ? <SkeletonPanel /> : <SignalList result={result} />}
                {result && result.signals.some(s => s.name === "vocabulary_novelty") && <VocabularyNoveltyCurve text={text} />}
              </div>
              
              {compareMode && (
                <div style={{ flex: 1 }}>
                  <h3 style={{ color: "#a78bfa", borderBottom: "1px solid rgba(139,92,246,0.2)", paddingBottom: "8px" }}>Revised Version</h3>
                  {loading === "scan" ? <SkeletonPanel /> : <SignalList result={compareResult} />}
                  {compareResult && compareResult.signals.some(s => s.name === "vocabulary_novelty") && <VocabularyNoveltyCurve text={compareText} />}
                </div>
              )}
            </div>
      ...
      {tab === "pr" && (
        <section className="workspace">
          <div className="panel formGrid">
            <div className="demo-presets-scan" style={{ marginBottom: "12px", gridColumn: "1 / -1" }}>
              <button className="demo-preset-btn" onClick={() => {
                setPrTitle("Improve billing retry behavior");
                setPrDescription("Updated billing files and improved retry handling. This fixes issues and enhances reliability.");
                setPrDiff("+ retryCount = Math.min(retryCount + 1, 3)\n+ idempotencyWindowMinutes = 10");
              }}>Slop PR</button>
              <button className="demo-preset-btn" onClick={() => {
                setPrTitle("Cap billing retries at 3 with 10m window");
                setPrDescription("We capped billing retries at 3 because Stripe replayed duplicate webhooks during deploys. Added a 10 minute idempotency window and tested replay fixtures for 200, 409, and timeout responses.");
                setPrDiff("+ retryCount = Math.min(retryCount + 1, 3)\n+ idempotencyWindowMinutes = 10\n+ // 10 minute window avoids deployment stampedes");
              }}>Genuine PR</button>
            </div>
